...
export function generatePDFReport(data, outputPath) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(10).font('Helvetica-Bold').text('[ P3L POS SYSTEM ]', 55, 52); // Logo placeholder
      doc.fontSize(18).font('Helvetica-Bold').text(data.metadata.business_name || 'FRESH FITY SUPERMARKET', 55, 65);
      doc.fontSize(9).font('Helvetica-Bold').fillColor(secondaryColor).text(
        `Business Report  |  Period: ${data.metadata.period_label}`, 
        55, 85
      );
      ...
      doc.fontSize(8).font('Helvetica-Bold').fillColor(secondaryColor).text('NET PROFIT', 305, y + 10);
      doc.fontSize(18).font('Helvetica-Bold').fillColor(accentColor).text(`KES ${data.metrics.total_profits.toLocaleString()}`, 305, y + 24);
      doc.fontSize(8).font('Helvetica').fillColor(secondaryColor).text('Money made after costs and expenses', 305, y + 46);

      y += rowHeight + 15;

      // Card 3: Cash & MPesa Breakdown
      doc.rect(40, y, colWidth, rowHeight).strokeColor(borderColor).stroke();
      doc.fontSize(8).font('Helvetica-Bold').fillColor(secondaryColor).text('PAYMENT TYPES', 50, y + 10);
      ...
      doc.rect(295, y, colWidth, rowHeight).strokeColor(borderColor).stroke();
      doc.fontSize(8).font('Helvetica-Bold').fillColor(secondaryColor).text('MONEY OUT (EXPENSES & REFUNDS)', 305, y + 10);
      doc.fontSize(10).font('Helvetica').fillColor(primaryColor).text(`Expenses: KES ${data.metrics.total_expenses.toLocaleString()}`, 305, y + 26);
      doc.text(`Refunds: KES ${data.metrics.refund_sales.toLocaleString()} (${data.metrics.refund_count} items)`, 305, y + 40);
      ...
      // Page Footer
      doc.strokeColor(borderColor).lineWidth(0.5).moveTo(40, 750).lineTo(540, 750).stroke();
      doc.fontSize(9).font('Helvetica-Bold').fillColor(secondaryColor).text(
        'Report Generated by P3L Developers API System. All rights reserved.', 40, 760
      );
      doc.fontSize(8).font('Helvetica').text(
        `Generated on: ${nowStr}`, 40, 775
      );
...
export async function emailReport(pdfPath, rangeLabel, recipientEmail) {
  ...
  const mailOptions = {
    from: `"P3L POS System" <${user}>`,
    to: recipientEmail || user,
    subject: `[P3L POS] Store Performance Summary (${rangeLabel})`,
    text: `Hello Admin,\n\nPlease find your daily sales products and business report attached here.\n\nRegards,\nRazak Guyo,\nAdmin P3L Developers.`,
    attachments: [
      {
...
// POST: Send a simple text-based summary via email (No Python required)
router.post('/send-summary', async (req, res) => {
  ...
  if (!user || !pass) {
    return res.status(500).json({ success: false, error: 'SMTP credentials not configured on the server.' });
  }
  ...
  try {
    ...
    const mailOptions = {
      from: `"P3L POS System" <${user}>`,
      to: email,
      subject: `End of Day Sales Summary - ${new Date().toLocaleDateString()}`,
      text: text
    };
    ...
    res.status(500).json({ success: false, error: 'Failed to send email. Check SMTP settings.' });
  }
});
